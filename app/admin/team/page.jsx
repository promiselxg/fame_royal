...
import { TeamTable } from "../_components/table/team/data-table";
import { TeamColumn } from "../_components/table/team/columns";
      ...
      <section className="w-full flex h-screen flex-col gap-y-5 p-5 overflow-y-scroll bg-[whitesmoke]">
        <div className="flex md:items-center md:justify-between w-full my-5  flex-col md:flex-row justify-start items-start">
          ...
          <Button className="border-none outline-none bg-[--button-bg] hover:bg-[--button-bg-hover] text-white transition-all delay-75 rounded-[5px]">
            <Link href="/admin/team/add" className="flex gap-2 items-center ">
              <Cross size={13} /> Add new team member
            </Link>
          </Button>
        </div>
        <TeamTable columns={TeamColumn} data={data} loading={loading} />
      </section>
    </>
  );
};
