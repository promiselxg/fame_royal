...
const EditBannerPage = ({ params }) => {
  ...
  const handleImageUpload = async (field, value) => {
    try {
      ...
      if (photos) {
        const response = await axios.put(`/api/gallary`, {
          id: params.id,
          field,
          value,
          photos,
  ...
  useEffect(() => {
    const getBannerData = async () => {
      try {
        const { data } = await axios.get(`/api/gallary/${params?.id}`);
        setData(data || {});
      } catch (error) {
        console.log(error);
      }
    ...
    <>
      <div className="h-screen w-full flex flex-col  overflow-y-scroll pb-[100px] md:pb-20">
        ...
        <div className="w-full my-5 bg-[whitesmoke] px-5 flex flex-col h-screen ">
          <div className="p-5">
            <h1 className={cn(`font-bold`)}>Edit photo gallary</h1>
          </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          {...field}
                          defaultValue={data?.gallary_title}
                          className="form-input"
                        />
                      </FormControl>
                      <FormDescription className="text-[12px] text-[#333]">
                        Give this photo a description.
                      </FormDescription>
                      <Button
                        type="button"
